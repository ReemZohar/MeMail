...
vector<shared_ptr<IAction>> addToBL(BloomFilter& bf) {
    ...
    for(shared_ptr<IUserInput> input : userInputs) {
        additions.push_back(ActionFactory::create(bf, input));
    }
...
TEST(DeleteURLFromBLTest, validDeletion) {
    ...
    set<string> expected3 = {"http://example.com/page?search=Exam#ple"};
    ...
    EXPECT_EQ(getBLFromBLFile(bfPath), bl);
...
TEST(DeleteURLFromBLTest, invalidDeletion) {
    ...
    DeleteURLFromBL del2 = DeleteURLFromBL(bf);
    fs::path bfPath = bf.getFilePath();
    vector<bool> originalBL = bf.getBlackList(), bl;
